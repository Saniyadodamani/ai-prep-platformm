    ...
    try {
      ...
      if (res.data.user && res.data.user.id) {
        ...
        if (res.data.user.name) {
          localStorage.setItem(`userName_${res.data.user.id}`, res.data.user.name);
        }
      }
  ...
  return (
    <div className="min-h-screen flex justify-center items-center bg-slate-950 text-white">
      <form
        ...
        className="bg-slate-900 p-8 rounded w-80"
      >
        ...
        <div className="relative w-full mb-4">
          <input
            ...
            className="w-full p-2 rounded bg-slate-800 pr-10"
            value={password}
